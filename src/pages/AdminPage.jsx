  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between bg-white rounded-xl shadow-sm p-4">
        <div>
          <h2 className="text-xl font-bold text-gray-900">Avis Google</h2>
          <p className="text-sm text-gray-500">Pilotage des avis affichés sur le site</p>
        </div>
        <button
          type="button"
          onClick={onRefresh}
          disabled={loading}
          className="px-4 py-2 bg-orange-500 text-white rounded-lg font-medium hover:bg-orange-600 disabled:opacity-60 transition-colors"
        >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-white rounded-xl p-6 shadow-sm border-l-4 border-orange-500">
          <div className="text-3xl font-bold text-orange-500">{stats.total_reviews || 0}</div>
          <div className="text-gray-600 text-sm">Avis total</div>
        </div>
        <div className="bg-white rounded-xl p-6 shadow-sm border-l-4 border-green-500">
          <div className="text-3xl font-bold text-green-500">{stats.average_rating || 0}</div>
          <div className="text-gray-600 text-sm">Note moyenne</div>
        </div>
        <div className="bg-white rounded-xl p-6 shadow-sm border-l-4 border-blue-500">
          <div className="text-3xl font-bold text-blue-500">{reviews.length}</div>
          <div className="text-gray-600 text-sm">Avis chargés</div>
        </div>
        <div className="bg-white rounded-xl p-6 shadow-sm border-l-4 border-purple-500">
          <div className="text-sm font-semibold text-gray-900 truncate">{stats.google_place_id || "-"}</div>
          <div className="text-gray-600 text-sm">Place ID</div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm p-6">
        <h3 className="text-lg font-bold text-gray-900 mb-4">Derniers avis</h3>
        <div className="space-y-3 max-h-[520px] overflow-y-auto">
          {reviews.map((review) => (
            <div key={review.id} className="border border-gray-200 rounded-lg p-4">
              <div className="flex justify-between items-start gap-3">
                ...
                <div className="text-sm font-semibold text-orange-600">{review.rating}/5</div>
              </div>
              <p className="mt-2 text-sm text-gray-700">{review.text}</p>
              {review.highlight && (
                <div className="mt-2 text-xs text-orange-700 bg-orange-50 px-2 py-1 rounded-md inline-block">
                  {review.highlight}
                </div>
              )}
            </div>
          ...
          {reviews.length === 0 && !loading && (
            <div className="text-center py-8 text-gray-500">Aucun avis disponible</div>
          )}
        </div>
      </div>
    </div>
...
const AdminPage = () => {
  ...
  const fetchReviews = async () => {
    try {
      setReviewsError("");
      const response = await fetch(`${API_URL}/api/reviews?limit=20`);
      if (!response.ok) {
        setReviewsError("Impossible de charger les avis Google");
        return;
      }
      const data = await response.json();
      setReviewsData(data);
    } catch (err) {
      setReviewsError("Erreur de connexion lors du chargement des avis");
    }
  };
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                Gérez votre contenu, vos messages et vos avis Google
                {refreshAt && (
                  <span className="ml-2 text-gray-400">• Mise à jour {refreshAt.toLocaleTimeString("fr-FR")}</span>
                )}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={handleRefresh} className="flex items-center gap-2 px-4 py-2 text-orange-700 bg-orange-100 hover:bg-orange-200 rounded-lg transition-colors">
