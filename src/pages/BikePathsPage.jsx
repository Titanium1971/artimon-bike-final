...
const ROUTE_SECTIONS = [
  ...
  { key: "easyIntermediate", routeIds: [2, 4] },
  { key: "longerLoops", routeIds: [8, 7, 1] },
  { key: "sportLoops", routeIds: [6] },
];

const SECTION_TITLES = {
  fr: {
    shortDiscovery: "Parcours court découverte",
    easyGreenway: "Voie verte facile",
    easyIntermediate: "Parcours facile intermédiaire",
    longerLoops: "Boucles plus longues",
    sportLoops: "Boucles sportives",
    selectedRoute: "Parcours sélectionné",
    otherRoutes: "Autres parcours",
  },
  en: {
    shortDiscovery: "Short Discovery Route",
    easyGreenway: "Easy Greenway",
    easyIntermediate: "Easy Intermediate Route",
    longerLoops: "Longer Loops",
    sportLoops: "Sport Loops",
...
const BikePathsPage = () => {
  ...
  const routes = useMemo(() => {
    ...
    const orderedIds = ROUTE_SECTIONS.flatMap((section) => section.routeIds);
  ...
  const displayNumberById = useMemo(
    () => new Map(routes.map((route, index) => [route.id, index + 1])),
    [routes]
  );

  const groupedRoutes = useMemo(() => {
    const routesById = new Map(filteredRoutes.map((route) => [route.id, route]));
    const selectedOnly = filterRoute !== "all";
    const sectionTitles = SECTION_TITLES[language] || SECTION_TITLES.fr;

    if (selectedOnly && filteredRoutes.length) {
      return [{ key: "selectedRoute", title: sectionTitles.selectedRoute, routes: filteredRoutes }];
    }

    const groups = ROUTE_SECTIONS.map((section) => {
      const sectionRoutes = section.routeIds.map((id) => routesById.get(id)).filter(Boolean);
      return {
        key: section.key,
    ...
    const includedIds = new Set(groups.flatMap((group) => group.routes.map((route) => route.id)));
    const remainingRoutes = filteredRoutes.filter((route) => !includedIds.has(route.id));
    if (remainingRoutes.length) {
      groups.push({ key: "otherRoutes", title: sectionTitles.otherRoutes, routes: remainingRoutes });
    }

    return groups;
  }, [filteredRoutes, filterRoute, language]);
            ...
            {groupedRoutes.map((group) => (
              <div key={group.key} className="space-y-8">
                <div className="flex items-center gap-4">
                  ...
                  <h2 className="text-xl md:text-2xl font-bold text-gray-900">{group.title}</h2>
                  <div className="h-px flex-1 bg-gray-200"></div>
                </div>
                    ...
                    >
                      <div className="p-6 md:p-8">
                        ...
                        <div className="flex items-center gap-4 mb-4">
                          ...
                          >
                            {displayNumberById.get(route.id) || route.id}
                          </span>
